...
void updateAltitudeDigits(int alt_ft, int delta_ft) {
  ...
  // instant‐update any non‐animated digits
  int anim = changed[0];
  for (int k = 0; k < n; k++) {
    int i = changed[k];
    if (i == anim) continue;
    int x = charX0 + i * charW;
    // clear only interior (pad1 above/below)
    const int pad1 = 4;
    int yClear = charY0 - pad1;
    int hClear = charH + pad1 * 2;
    tft.fillRect(x, yClear, charW, hClear, BLACK);
    tft.setCursor(x, charY0);
    tft.print(newStr[i]);
    prevAltStr[i] = newStr[i];
  }

  // animate the one column
  int i   = anim;
  int x0  = charX0 + i * charW;
  int y0  = charY0;
  char oldC = prevAltStr[i], newC = newStr[i];
  const int frames = 8;
  const int pad1   = 4;
  int clearY = y0 - pad1;
  int clearH = charH + pad1 * 2;

  for (int f = 0; f <= frames; f++) {
    // clear interior
    tft.fillRect(x0, clearY, charW, clearH, BLACK);
  ...
  // touch up any tiny gaps once
  drawDigitOutline();
}

// ——— Draw the 8‐segment outline ———
void drawDigitOutline(){
  const int pad1 = 4, pad2 = 20;
  int x0 = charX0 - 1;
  int x1 = charX0 + 2*charW - 1;
  int x2 = charX0 + 3*charW + 1;
  int y1 = charY0 - pad1 - 1;
  int y2 = charY0 - pad2 - 1;
  int y3 = charY0 + charH + pad1 + 1;
  int y4 = charY0 + charH + pad2 + 1;
